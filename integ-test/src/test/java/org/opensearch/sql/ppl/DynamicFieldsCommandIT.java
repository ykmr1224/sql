 ...
 *
 * <p>Tests the new `dynamic=true` parameter syntax that enables collection of fields not defined in
 * schema into a _MAP column as Map<String, Any>.
 */
public class DynamicFieldsCommandIT extends PPLIntegTestCase {
  ...
  public void testDynamicParameterFalse() throws IOException {
    assertThrows(
        ...
        () ->
            executeQuery(
                String.format(
                    "source=%s dynamic=false | fields firstname, lastname, department | head 1",
                    TEST_INDEX_DYNAMIC)));
  }
