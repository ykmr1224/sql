...
public class CalciteRexNodeVisitor extends AbstractNodeVisitor<RexNode, CalcitePlanContext> {
  ...
  public RexNode visitQualifiedName(QualifiedName node, CalcitePlanContext context) {
    RexNode mapField = context.relBuilder.field(LogicalExpandJson.MAP_FIELD_NAME);
    return context.rexBuilder.makeCall(
        SqlStdOperatorTable.ITEM, mapField, context.rexBuilder.makeLiteral(node.toString()));
    ...
    //     // `n2.n_name` was the renamed field name from the duplicated field `(nation2.)n_name0`
    // of
    //     // join output.
