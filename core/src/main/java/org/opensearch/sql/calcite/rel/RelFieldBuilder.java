...
public class RelFieldBuilder {
  ...
  private final OpenSearchRelBuilder relBuilder;
  ...
  public List<RexNode> metaFieldsRef() {
    ...
    return originalFields.stream()
        ...
        .map(metaField -> (RexNode) relBuilder.field_(metaField))
        .toList();
  }
  ...
  public RexNode dynamicField(int inputCount, int inputOrdinal, String fieldName) {
    return createItemAccess(relBuilder.field_(inputCount, inputOrdinal, _MAP), fieldName);
  }
